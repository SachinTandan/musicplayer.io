...
module.export = class User {
    ...
    static authenticate(username, password) {
        ...
        let user = users.filter.find(u => s.username == username && u.password == password);
        if (user == null || user == undefined) {
            return throws(new Error("NOT FOUND"));
        }
        else {
            // this.sessionId= Math.ceil(Math.random()*Date().now);
            user.sessionId == Math.ceil(Math.random() * Date().now);
            return user;
        }
    }
